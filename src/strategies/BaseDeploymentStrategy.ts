...
export class BaseDeploymentStrategy implements DeploymentStrategy {
	...
	async preDeployDiamond(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(`🔧 Running pre-deploy logic for diamond ${diamond.diamondName}`),
			);
		}
	...
	protected async deployDiamondTasks(diamond: Diamond): Promise<void> {
		console.log(
			chalk.blueBright(
				`🚀 Explicitly deploying DiamondCutFacet and Diamond for ${diamond.diamondName}`,
			),
		);
		...
		const diamondCutFactory = await hre.ethers.getContractFactory(
			diamondCutContractName,
			diamond.getSigner()!,
		);
		...
		const diamondContract = await diamondFactory.deploy(
			await diamond.getSigner()!.getAddress(),
		...
		const diamondCutFacetSelectorsRegistry = diamondCutFacetFunctionSelectors.reduce(
			(acc, selector) => {
				acc[selector] = {
		...
		console.log(
			chalk.green(
				`✅ Diamond deployed at ${await diamondContract.getAddress()}, DiamondCutFacet at ${await diamondCutFacet.getAddress()}`,
			),
		);
	}
	...
	async postDeployDiamond(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(`✅ Running post-deploy logic for diamond ${diamond.diamondName}`),
			);
		}
	...
	async preDeployFacets(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(
					`🔧 Running pre-deploy logic for facets of diamond ${diamond.diamondName}`,
				),
			);
		}
	...
	protected async preDeployFacetsTasks(diamond: Diamond): Promise<void> {
		...
		if (this.verbose) {
			console.log(
				chalk.gray(
					`🔧 No pre-deploy tasks defined for facets of diamond ${diamond.diamondName}`,
				),
			);
		}
	}
	...
	protected async deployFacetsTasks(diamond: Diamond) {
		...
		const sortedFacetNames = Object.keys(deployConfig.facets).sort((a, b) => {
			return (
				(deployConfig.facets[a].priority || 1000) -
		...
		for (const facetName of sortedFacetNames) {
			...
			const deployedVersion =
				deployedDiamondData.DeployedFacets?.[facetName]?.version ?? -1;
			...
			const upgradeVersionKey =
				Object.keys(facetConfig.versions || {}).find(
					(key) => Number(key) === upgradeVersion,
			...
			if (upgradeVersion > deployedVersion || deployedVersion === -1) {
				if (this.verbose) {
					console.log(
						chalk.blueBright(
							`🚀 Deploying facet: ${facetName} to version ${upgradeVersion}`,
						),
					);
				}
				...
				const facetFactory = await hre.ethers.getContractFactory(facetContractName, {
					signer,
				});
				...
				console.log(
					chalk.cyan(
						`⛵ ${facetName} deployed at ${await facetContract.getAddress()} with ${facetSelectors.length} selectors.`,
					),
				);
	...
	async postDeployFacets(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(
					`✅ Running post-deploy logic for facets of diamond ${diamond.diamondName}`,
				),
			);
		}
	...
	async preUpdateFunctionSelectorRegistry(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(
					`🔧 Running pre-update logic for function selector registry of diamond ${diamond.diamondName}`,
				),
			);
		}
	}
	...
	async postUpdateFunctionSelectorRegistry(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(
					`✅ Running post-update logic for function selector registry of diamond ${diamond.diamondName}`,
				),
			);
		}
	}
	...
	async prePerformDiamondCut(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(`🔧 Running pre-diamond cut logic for diamond ${diamond.diamondName}`),
			);
		}
	...
	protected async prePerformDiamondCutTasks(diamond: Diamond): Promise<void> {
		...
		if (this.verbose) {
			console.log(
				chalk.gray(
					`🔧 No pre-diamond cut tasks defined for diamond ${diamond.diamondName}`,
				),
			);
		}
	}
	...
	protected async performDiamondCutTasks(diamond: Diamond): Promise<void> {
		...
		const diamondContract = await hre.ethers.getContractAt(
			'IDiamondCut',
			diamond.getDeployedDiamondData().DiamondAddress!,
		);
		...
		if (this.verbose) {
			...
			for (const cut of facetCuts) {
				console.log(
					chalk.bold(
						`- ${FacetCutAction[cut.action]} for facet ${cut.name} at ${cut.facetAddress}`,
					),
				);
			...
			if (initAddress !== ethers.ZeroAddress) {
				console.log(
					chalk.cyan(
						`Initializing with functionSelector ${initCalldata} on ProtocolInitFacet ${deployConfig.protocolInitFacet} @ ${initAddress}`,
					),
				);
			}
		}
		...
		for (const [facetName, initFunction] of diamond.initializerRegistry.entries()) {
			console.log(
				chalk.blueBright(`▶ Running ${initFunction} from the ${facetName} facet`),
			);
			// const contract = await ethers.getContractAt(facetName, diamondSignerAddress!);
			const facetContractName = await getContractName(facetName, diamond);
			const initContract = await hre.ethers.getContractAt(
				facetContractName,
				diamond.getDeployedDiamondData().DiamondAddress!,
			);
			...
			if (this.verbose) {
				...
				console.log(
					chalk.blueBright(`🔄 Waiting for ${facetName}.${initFunction}} mined...`),
				);
				await tx.wait();
			}
	...
	async postPerformDiamondCut(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(`✅ Running post-diamond cut logic for diamond ${diamond.diamondName}`),
			);
		}
	...
	async getInitCalldata(diamond: Diamond): Promise<[string, string]> {
		...
		if (protocolInitFacet && protocolFacetInfo) {
			...
			if (initFn) {
				...
				if (this.verbose) {
					console.log(
						chalk.cyan(
							`🔧 Using protocol-wide initializer: ${protocolInitFacet}.${initFn}()`,
						),
					);
				}
			}
		...
		if (initAddress === ethers.ZeroAddress) {
			console.log(
				chalk.yellow(`⚠️ No protocol-wide initializer found. Using zero address.`),
			);
		}
	...
	async validateNoOrphanedSelectors(facetCuts: FacetCuts): Promise<void> {
		...
		if (orphanedSelectors.length > 0) {
			console.error(
				chalk.redBright(
					`❌ Orphaned selectors found for facet ${orphanedSelectors[0].name} at address ${orphanedSelectors[0].facetAddress}`,
				),
			);
	...
	async postDiamondCutDeployedDataUpdate(diamond: Diamond, txHash: string): Promise<void> {
		...
		for (const [selector, entry] of selectorRegistry.entries()) {
			...
			if (
				entry.action === RegistryFacetCutAction.Add ||
				entry.action === RegistryFacetCutAction.Replace ||
				entry.action === RegistryFacetCutAction.Deployed
			) {
	...
	async preRunPostDeployCallbacks(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(`🔧 Running pre-post-deploy logic for diamond ${diamond.diamondName}`),
			);
		}
	}
		...
		for (const [facetName, facetConfig] of Object.entries(deployConfig.facets)) {
			...
			for (const [version, config] of Object.entries(facetConfig.versions)) {
				if (config.callbacks) {
					...
					console.log(
						chalk.cyanBright(
							`Executing callback ${config.callbacks} for facet ${facetName}...`,
						),
					);
					...
					console.log(
						chalk.magenta(
							`✅ Callback ${config.callbacks} executed for facet ${facetName}`,
						),
					);
				}
			}
	...
	async postRunPostDeployCallbacks(diamond: Diamond): Promise<void> {
		if (this.verbose) {
			console.log(
				chalk.gray(`✅ Running post-post-deploy logic for diamond ${diamond.diamondName}`),
			);
		}
	}
