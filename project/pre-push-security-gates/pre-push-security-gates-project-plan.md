...
| **M2** | Semgrep Gate: Triage & Disposition | `yarn semgrep:scan` green (10 `typescript-any-usage` fixed/annotated/tuned) | Low-Med — one CLI file + maybe `.semgrep.yml` |
...
**Critical path:** `M0 → M1 → M2 → M3 → M4`. M1/M2/M3 clear individual blocking gates and can be reviewed independently, but the **final acceptance (unbypassed push)** lives in M4 and requires all prior gates green. M3's slither removal cites the M0 tooling-repo rationale (and M2 would too if any Solidity rule fired — currently none do; all 10 semgrep findings are `typescript-any-usage`).
...
### M2 — Semgrep Gate: Triage & Disposition
...
| Epic | Title | Summary | Owner | Impact |
|------|-------|---------|-------|--------|
| M2-E1 | Enumerate & classify | List all 10 `typescript-any-usage` findings (`src/cli/diamond-abi-cli.ts` lines 348–479); classify each as *fixable-with-a-real-type* vs *justified-any*; decide the disposition (fix / annotate / tune) | Engineer | Reference |
| M2-E2 | Disposition → gate green | Apply the decision: replace fixable `any`s with proper types, add inline `// nosemgrep: typescript-any-usage` (with reason) for justified ones, and/or tune `typescript-any-usage` in `.semgrep.yml`; re-run `yarn semgrep:scan` to **0 blocking** | Engineer | Med |

*(Reconciled from the plan's original 3 M2 epics: the "scope non-applicable Solidity rules" epic was dropped — the M0 baseline found **no** Solidity-rule-on-fixture findings; all 10 are `typescript-any-usage`. If Solidity rules ever fire, the `.semgrep.yml` `paths`-scoping mechanism + the M0 tooling-repo rationale still apply.)*
