...
const config: HardhatUserConfig = {
	...
	networks: {
		hardhat: {
			...
			forking:
				process.env.NODE_ENV === 'test' && TEST_NETWORK && TEST_NETWORK !== 'hardhat'
					? {
							url: getNetworkUrl(TEST_NETWORK),
							blockNumber: undefined, // Use latest block
						}
					: undefined,
			gas: parseInt(MAX_GAS_LIMIT ?? '8000000'),
			gasPrice: parseInt(MAX_GAS_PRICE ?? '100') * 1e9, // Convert Gwei to Wei
		},

		mainnet: {
			url: MAINNET_RPC_URL ?? 'https://mainnet.infura.io/v3/YOUR_INFURA_KEY',
			accounts: [DEPLOYER_PRIVATE_KEY ?? DEFAULT_TEST_PRIVATE_KEY],
			chainId: 1,
			gas: parseInt(MAX_GAS_LIMIT ?? '8000000'),
			gasPrice: parseInt(MAX_GAS_PRICE ?? '100') * 1e9,
		},
		sepolia: {
